...
public class InfoView extends Activity {
  private static TextView tvAppName = null;
  private static TextView tvAppVers = null;
  private static TextView tvDataInfo = null;
  private static Button btnBuyApp;
  ...
  public void onCreate(Bundle savedInstanceState) {
    ...
    try {
      ...
      // obj ref for setting the application name
      InfoView.tvAppName = (TextView) findViewById(R.id.appname);
      InfoView.tvDataInfo = (TextView) findViewById(R.id.txtdatainfo);

      // set the appliction version
      InfoView.tvAppVers = (TextView) findViewById(R.id.appversion);
      InfoView.tvAppVers.setText(this.getPackageManager().getPackageInfo(
          this.getPackageName(), 0).versionName);

      InfoView.tvAppName.setText(getString(R.string.app_name));
      InfoView.tvDataInfo.setText(getString(R.string.info_note));
    }// end try statements
    catch (Exception error) {
      MyErrorLog<Exception> errExcpError = new MyErrorLog<Exception>(this);
      errExcpError.addToLogFile(error, "InfoView.onCreate", "");
      errExcpError = null;
  ...
  public boolean onMenuItemSelected(int featureId, MenuItem item) {
    ...
    case R.id.quit:
      ...
      /* perform cleanup */
      Intent intentQuitActivity = new Intent(InfoView.this, Music_List.class);
  ...
  protected void onResume() {
    ...
    try {
      ...
      String strPkgName = this.getPackageName();
      int intSPos = strPkgName.length() - ("Demo").length();
      int intEPos = strPkgName.length();
      String strEndOfPkgName = strPkgName.substring(intSPos, intEPos);
      if (strEndOfPkgName.equals("Demo")) {
        // display demo version info
        InfoView.tvAppName.setText(getString(R.string.app_demo_name));
        InfoView.tvDataInfo.setText(getString(R.string.info_note));
      } else {
        // display paid version info
        InfoView.tvAppName.setText(getString(R.string.app_name));
        InfoView.tvDataInfo.setText(getString(R.string.info_note));

        // hide the button
        btnBuyApp.setEnabled(false);
        btnBuyApp.setVisibility(Button.INVISIBLE);
      }// end if (strEndOfPkgName.equals("Demo"))
    } catch (Exception error) {
      MyErrorLog<Exception> errExcpError = new MyErrorLog<Exception>(
          InfoView.this);
      errExcpError.addToLogFile(error, "InfoView.onResume", "no prompt");
      errExcpError = null;
    }// end try/catch (Exception error)
  ...
  protected void onStop() {
    try {
      ...
      MyErrorLog<Exception> errExcpError = new MyErrorLog<Exception>(
          InfoView.this);
      errExcpError.addToLogFile(error, "InfoView.onStop", "no prompt");
      errExcpError = null;
    }// end try/catch (Exception error)
  }// end onStop
